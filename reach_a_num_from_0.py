# Question:
#   Given a number, reach the number from 0 by performing any of the 3 operations.
#   1) Addition of 1
#   2) Subtracting 1
...
# Solution: we go from number to 0 instead.
# 1.When the num is even, then only option we have to reach num/2 is sub and
...
def reach_zero(num):
  ...
  while(num):
    ...
    # If number is odd we have to go to even num, keeping extra_cost in mind.
    if num%2 == 1:
